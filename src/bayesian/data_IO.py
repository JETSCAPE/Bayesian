    ...
    for filename in os.listdir(prediction_dir):
        if "values" in filename and parameterization in filename:
            if _accept_observable(analysis_config, filename):
                ...
                observables["Prediction"][observable_label]["y_err"] = np.take(
                    prediction_errors, training_indices, axis=1
                )

                observables["Prediction_validation"][observable_label]["y"] = np.take(
                    prediction_values, validation_indices, axis=1
                )
                observables["Prediction_validation"][observable_label]["y_err"] = np.take(
                    prediction_errors, validation_indices, axis=1
                )
...
def data_array_from_h5(
    output_dir, filename, pseudodata_index: int = -1, observable_filter: ObservableFilter | None = None
):
    """
    ...
    if not sorted_observable_list:
        logger.warning(
            "No observables passed the filter. Check observable_list / observable_exclude_list in your config."
        )
    ...
    if pseudodata_index < 0:
        ...
        for i, observable_label in enumerate(sorted_observable_list):
            ...
            data_dict[observable_label]["y"] = prediction_central_value + np.random.normal(
                loc=0.0, scale=exp_uncertainty
            )
