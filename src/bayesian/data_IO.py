...
def _read_grouped_columns(filepath, grouped_columns):
    ...
    for base, indices in grouped_columns.items():
        ...
        if not valid:
            logger.warning(
                f"Column '{base}' at indices {indices} not in {filepath} (only {full_data.shape[1]} columns)"
            )
            continue
        if len(valid) == 1:
...
def initialize_observables_dict_from_tables(
    ...
    for filename in os.listdir(data_dir):
        ...
        if _accept_observable(analysis_config, filename):
            ...
            systematic_data = {k: v for k, v in all_columns.items() if k not in _STAT_COLUMN_ALIASES}
