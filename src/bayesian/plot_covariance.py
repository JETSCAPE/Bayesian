...
def plot_all_covariance_components(
    config: mcmc.MCConfig,
    output_dir: str,
    parameter_point: np.ndarray | None = None,
    ...
    if parameter_point is None:
        ...
        try:
            ...
            try:
                ...
                parameter_point = np.zeros(
                    len(config.analysis_config["parameterization"][config.parameterization]["names"])
                )
...
def _extract_covariance_components(
    ...
    config: mcmc.MCConfig,  # ADD config parameter
) -> dict[str, np.ndarray]:
    ...
    if "correlation_manager" in experimental_data:
        cov_components.update(
            _compute_systematic_group_covariances(
    ...
    cov_components["total"] = (
        cov_components["statistical"] + cov_components["systematic_total"] + cov_components["emulator"]
    )
...
def _plot_covariance_comparison(
    cov_components: dict[str, np.ndarray], plot_dir: Path, figsize: tuple[int, int], cmap: str
) -> None:
    """
...
def _plot_individual_covariance_matrices(cov_components: dict[str, np.ndarray], plot_dir: Path, cmap: str) -> None:
    """
...
def _plot_systematic_group_correlations(
    cov_components: dict[str, np.ndarray], plot_dir: Path, cmap: str = "RdBu_r"
) -> None:
    """
    ...
    systematic_groups = []
    for comp_name in cov_components:
    ...
    for group_name, comp_name in systematic_groups:
        ...
        # Covariance matrix
        _plot_single_matrix(
            cov_matrix, ax_cov, f'Systematic Group "{group_name}" Covariance', cmap, matrix_type="covariance"
        )

        # Correlation matrix with improved visualization
        _plot_single_matrix(
            corr_matrix, ax_corr, f'Systematic Group "{group_name}" Correlation', "RdBu_r", matrix_type="correlation"
        )
    ...
    if len(systematic_groups) > 1:
        ...
        for i, (group_name, comp_name) in enumerate(systematic_groups):
            ...
            corr_matrix = _covariance_to_correlation(cov_matrix)
            _plot_single_matrix(corr_matrix, axes[i], f'Group "{group_name}"', "RdBu_r", matrix_type="correlation")
...
def _plot_correlation_matrices(cov_components: dict[str, np.ndarray], plot_dir: Path, cmap: str) -> None:
    """
    ...
    for i, comp_name in enumerate(correlation_components):
        ...
        _plot_single_matrix(
            corr_matrix, axes[i], f"{comp_name.replace('_', ' ').title()}", "RdBu_r", matrix_type="correlation"
        )
...
def _plot_single_matrix(
    matrix: np.ndarray, ax: plt.Axes, title: str, cmap: str, matrix_type: str = "covariance"
) -> None:
    """
...
def _plot_covariance_diagnostics(cov_components: dict[str, np.ndarray], plot_dir: Path) -> None:
    """
...
def _get_emulator_predictions(config: mcmc.MCConfig, parameter_point: np.ndarray) -> dict:
    ...
    try:
        ...
        predictions = base.predict(parameter_point, emulation_config, emulation_group_results=emulation_results)
...
def plot(analysis_name: str, parameterization: str, analysis_config: dict[str, Any], config_file: str) -> None:
    """
    ...
    mcmc_config = mcmc.MCConfig.from_config_file(analysis_settings=analysis_settings)
...
def plot_covariance_analysis(config: mcmc.MCConfig, output_dir: str) -> None:
    """
