...
def plot(config: mcmc.MCConfig):
    ...
    logger.info(
        f"Plotting MCMC results for chain with n_walkers={n_walkers}, n_sampling_steps={n_sampling_steps}, n_dim={n_dim}"
    )
...
def _plot_autocorrelation_time(results, plot_dir, config):
    ...
    mean_autocorrelation_time = np.concatenate(
        (mean_autocorrelation_time_parameters, mean_autocorrelation_time_posterior)
    )
    std_autocorrelation_time = np.concatenate((std_autocorrelation_time_parameters, std_autocorrelation_time_posterior))
...
def _plot_design_pairplot(design, plot_dir, config):
    ...
    for col in df.columns:
        if "c_" in col:
            ...
            df.rename(columns={col: col.replace("c_", r"\mathrm{ln}c_")}, inplace=True)
